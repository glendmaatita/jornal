...
routerAdd("GET", "/api/jornal/invoicing/invoices/{id}", (event) => {
  ...
  return event.json(200, { invoice: h.invoiceResponseWithContactFallback($app, invoice), payment: payment ? h.paymentResponse(payment) : null })
}, $apis.requireAuth())
...
routerAdd("GET", "/api/jornal/invoicing/invoices/{id}/document", (event) => {
  ...
  const result = h.invoiceResponseWithContactFallback($app, invoice); return event.json(200, { documentVersion: invoice.getString("template_version"), contentHash: $security.sha256(h.stableStringify({ stored: invoice.getString("content_hash"), sender: result.senderSnapshot })), invoice: result })
}, $apis.requireAuth())
