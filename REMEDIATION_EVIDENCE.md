...
| Share target body bound | Live `PORT=4183 bun run server.ts`; 9 MB multipart upload | Request is rejected early with `413` before intake; text fields are capped before storage |
...
- Bootstrap now distinguishes an unavailable configured PocketBase from an empty account and routes authenticated users to a retryable data-unavailable screen instead of onboarding (F14).
...
- PocketBase runtime directories now contain only `.js` migrations/hooks; Bun/TypeScript test files live under `backend/pocketbase/tests`, preventing PocketBase startup from parsing test files as migrations. The two moved invariant tests still pass.
