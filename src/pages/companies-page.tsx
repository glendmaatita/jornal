...
import { Archive, Building2, Check, Plus, RotateCcw } from "lucide-react"
          ...
          <Card key={company.id} className={company.id === current?.id ? "border-[#df1769]/50" : ""}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-secondary"><Building2 className="size-5" /></span><div className="min-w-0"><p className="truncate font-semibold">{company.name}</p><p className="text-xs text-muted-foreground">{company.status === "ARCHIVED" ? "Diarsipkan" : company.id === current?.id ? "Company aktif" : "Aktif"}{pendingCounts[company.id] ? ` · ${pendingCounts[company.id]} perubahan tertunda` : ""}</p></div></div>
