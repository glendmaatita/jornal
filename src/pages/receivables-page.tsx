  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <Card className="border-[#e2a9cb]/60 bg-[var(--main-dark)] text-white">
        <CardContent className="p-5"><p className="text-xs font-semibold tracking-[1.5px] text-white/70 uppercase">Total piutang berjalan</p><p className="mt-1 text-3xl font-bold tabular-nums">{formatRupiah(total)}</p><p className="mt-1 text-xs text-white/70">{open.length} orang/transaksi masih menunggak</p></CardContent>
      </Card>
      {receivables.length === 0 ? <Card className="border-dashed"><CardContent className="p-8 text-center"><p className="text-sm text-muted-foreground">Belum ada piutang.</p>{writable && <Link to="/add" search={{ receivable: "new" }} className="mt-3 inline-block text-sm font-semibold text-[var(--link)]">Catat uang yang dipinjamkan</Link>}</CardContent></Card> : (
        <Card><CardContent className="divide-y divide-border/70 p-0">{receivables.map(({ transaction, paid, outstanding }) => {
          ...
          return <div key={transaction.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><Link to="/transactions/$transactionId" params={{ transactionId: transaction.id }} className="font-semibold hover:underline">{transaction.supplierCustomer || transaction.description}</Link><p className="mt-0.5 text-xs text-muted-foreground">Dipinjamkan {formatDateShort(transaction.transactionDate)}{transaction.receivableDueDate ? ` · jatuh tempo ${formatDateShort(transaction.receivableDueDate)}` : ""}</p></div><p className={overdue ? "text-sm font-bold text-red-600" : "text-sm font-bold text-[var(--main-dark)]"}>{outstanding > 0 ? formatRupiah(outstanding) : "Lunas"}</p></div><div className="mt-3 flex items-center justify-between text-xs text-muted-foreground"><span>Dibayar {formatRupiah(paid)} dari {formatRupiah(transaction.amount)}</span>{writable && outstanding > 0 && <Link to="/add" search={{ receivable: transaction.id }} className="font-semibold text-[var(--link)]">Catat pembayaran</Link>}</div></div>
        })}</CardContent></Card>
      )}
    </div>
  )
