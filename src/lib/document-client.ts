...
function send<T>(path: string, options: { method?: string; body?: unknown } = {}) { return pb.send<T>(path, { ...options, headers: { "X-Jornal-Protocol": "3" } }) }
...
export function listDocuments(status?: string) { return send<{ items: ServerDocument[] }>(`/api/jornal/documents?${params()}${status ? `&status=${encodeURIComponent(status)}` : ""}`) }
export function searchDocuments(search: string) { return send<{ items: Array<{ document: ServerDocument; summary: { documentType: string | null; merchantName: string | null; description: string | null } }> }>(`/api/jornal/documents/search?${params()}&search=${encodeURIComponent(search)}`) }
export function listDocumentInvoiceCandidates(id: string, amount: number) { return send<{ items: Invoice[] }>(`/api/jornal/documents/${encodeURIComponent(id)}/invoice-candidates?${params()}&amount=${amount}`) }
export function getDocument(id: string) { return send<{ document: ServerDocument; linkedInvoiceId: string | null; contentBase64: string; extraction: { id: string; result: DocumentExtraction; warnings: string[]; model: string; createdAt: string } | null }>(`/api/jornal/documents/${encodeURIComponent(id)}?${params()}`) }
export function extractDocument(id: string) { return send<{ job: { id: string; status: string } }>(`/api/jornal/documents/${encodeURIComponent(id)}/extract`, { method: "POST", body: { ...scope(), requestKey: crypto.randomUUID() } }) }
export function getAiJob(id: string) { return send<{ job: { id: string; documentId: string; status: string; errorCode: string | null; result: DocumentExtraction | null } }>(`/api/jornal/ai-jobs/${encodeURIComponent(id)}?${params()}`) }
export function archiveDocument(document: ServerDocument) { return send(`/api/jornal/documents/${encodeURIComponent(document.id)}/archive`, { method: "POST", body: { ...scope(), expectedRevision: document.revision } }) }
export async function confirmDocument(document: ServerDocument, input: { mode?: "CREATE_TRANSACTION" | "LINK_TRANSACTION" | "MATCH_INVOICE"; transactionId?: string; expectedTransactionRevision?: number; invoiceId?: string; expectedInvoiceRevision?: number; direction?: "MONEY_IN" | "MONEY_OUT"; amount?: number; transactionDate?: string; description?: string; accountId?: string | null }) { const result = await send<{ document: ServerDocument; ledgerTransaction: Transaction; ledgerRevision: number }>(`/api/jornal/documents/${encodeURIComponent(document.id)}/confirm`, { method: "POST", body: { ...scope(), ...input, mode: input.mode || "CREATE_TRANSACTION", transactionId: input.transactionId || crypto.randomUUID(), expectedRevision: document.revision, commandKey: crypto.randomUUID() } }); reconcileServerTransaction(result.ledgerTransaction); acceptServerTransactionRevision(result.ledgerTransaction.id, result.ledgerRevision); return result }
export async function unlinkDocument(document: ServerDocument, reason: string) { const result = await send<{ document: ServerDocument; ledgerTransaction: Transaction; ledgerRevision: number; ledgerDeleted: boolean }>(`/api/jornal/documents/${encodeURIComponent(document.id)}/unlink`, { method: "POST", body: { ...scope(), expectedRevision: document.revision, reason, commandKey: crypto.randomUUID() } }); if (result.ledgerDeleted) reconcileServerTransactionDeletion(result.ledgerTransaction); else reconcileServerTransaction(result.ledgerTransaction); acceptServerTransactionRevision(result.ledgerTransaction.id, result.ledgerRevision); return result }
