...
export async function unlinkDocument(document: ServerDocument, reason: string) { const result = await send<{ document: ServerDocument; ledgerTransaction: Transaction; ledgerDeleted: boolean }>(`/api/jornal/documents/${encodeURIComponent(document.id)}/unlink`, { method: "POST", body: { ...scope(), expectedRevision: document.revision, reason, commandKey: crypto.randomUUID() } }); if (result.ledgerDeleted) reconcileServerTransactionDeletion(result.ledgerTransaction); else reconcileServerTransaction(result.ledgerTransaction); return result }
