...
import { formatDateShort, formatPeriodLabel, formatRupiah, parseAmountInput, todayIsoDate } from "@/lib/format"
...
function dateLabel(value: string | null) { const date = dateOnly(value); return date ? formatDateShort(date) : null }
  ...
  return (
    <Card>
      <CardContent className="space-y-4 p-5">
        ...
        <div className="grid grid-cols-2 gap-2"><SelectField label="Awal tahun buku—bulan" value={fiscalMonth} onChange={setFiscalMonth} options={Array.from({ length: 12 }, (_, index) => ({ value: String(index + 1), label: String(index + 1) }))} /><TextField label="Awal tahun buku—tanggal" value={fiscalDay} onChange={setFiscalDay} /></div>
        <SelectField label="Kelayakan PPh Final UMKM" value={umkmEligibility} onChange={(value) => setUmkmEligibility(value as typeof umkmEligibility)} options={[{ value: "NEEDS_REVIEW", label: "Perlu diperiksa" }, { value: "ELIGIBLE", label: "Sudah dikonfirmasi memenuhi syarat" }, { value: "INELIGIBLE", label: "Tidak memenuhi syarat" }]} />
...
function TaxSettings({ subject, membership, registrations, preference, busy, run }: {
  ...
  return <Card><CardContent className="p-5">
    ...
    {open && <div className="mt-4 space-y-5 border-t border-border pt-4">
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]"><SelectField label="Kelayakan PPh Final UMKM" value={eligibility} onChange={(value) => setEligibility(value as typeof eligibility)} options={[{ value: "NEEDS_REVIEW", label: "Perlu diperiksa" }, { value: "ELIGIBLE", label: "Memenuhi syarat—dikonfirmasi" }, { value: "INELIGIBLE", label: "Tidak memenuhi syarat" }]} /><Button className="self-end" variant="outline" disabled={busy || eligibility === subject.umkmEligibility} onClick={saveIdentity}>Simpan status</Button></div>
      {available.length > 0 && <div className="space-y-2 rounded-xl border border-border p-3"><p className="text-sm font-semibold">Tambah jenis pajak</p><SelectField aria-label="Jenis pajak" value={kind} onChange={(value) => setKind(value as TaxKind)} options={available.map((option) => ({ value: option.kind, label: option.label }))} /><div className="grid gap-2 sm:grid-cols-3"><TextField label="Nominal default (opsional)" type="amount" prefix="Rp" value={amount} onChange={setAmount} /><DateField label={selectedOption?.event ? "Jatuh tempo dokumen" : "Tanggal manual (opsional)"} value={dueDate} onChange={setDueDate} /><TextField label="Daerah/sumber" value={jurisdiction} onChange={setJurisdiction} /></div><Button disabled={busy} onClick={addRegistration}><Plus className="size-4" />Tambah kewajiban</Button></div>}
...
function PeriodReconciliation({ subjectId, companyId, transactions, busy, run }: {
  ...
  }, `Masa ${formatPeriodLabel(period)} direkonsiliasi dan agenda diperbarui.`).then(() => setOpeningYtd(""))
  return <Card><CardContent className="space-y-3 p-5"><div><h2 className="flex items-center gap-2 text-lg"><CalendarClock className="size-4 text-primary" />Rekonsiliasi {formatPeriodLabel(period)}</h2><p className="text-xs text-muted-foreground">Omzet company di Jornal: {formatRupiah(ledgerRevenue)}. Konfirmasi omzet usaha lain agar batas kumulatif tidak keliru.</p></div><MonthField label="Masa pajak" max={currentPeriod()} value={period} onChange={setPeriod} /><TextField label="Omzet usaha di luar company Jornal bulan ini" type="amount" prefix="Rp" value={external} onChange={setExternal} /><TextField label="Saldo omzet YTD sebelum masa pertama di Jornal (isi sekali)" type="amount" prefix="Rp" value={openingYtd} onChange={setOpeningYtd} hint="Termasuk omzet company/usaha lain sebelum Anda mulai merekonsiliasi di Jornal." /><Button disabled={busy || !/^\d{4}-\d{2}$/.test(period)} onClick={reconcile}>{busy ? "Memproses…" : "Konfirmasi dan buat agenda"}</Button></CardContent></Card>
}
...
function AgendaList({ obligations, filings, companyId, busy, run }: {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap gap-2" aria-label="Filter agenda pajak">{(["ALL", "MONTHLY", "ANNUAL", "EVENT"] as const).map((value) => <Button key={value} size="sm" variant={scope === value ? "default" : "outline"} onClick={() => setScope(value)}>{value === "ALL" ? "Semua" : value === "MONTHLY" ? "Bulanan" : value === "ANNUAL" ? "Tahunan" : "Lainnya"}</Button>)}{years.length > 1 && <SelectField aria-label="Tahun agenda" size="compact" value={year} onChange={setYear} options={[{ value: "ALL", label: "Semua tahun" }, ...years.map((item) => ({ value: item, label: item }))]} />}</div>
...
function ObligationRow({ item, companyId, busy, run }: { item: TaxObligation; companyId: string; busy: boolean; run: (work: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  const due = dateLabel(item.effectiveDueDate)
  return <article className="rounded-xl border border-border p-3"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{KIND_LABEL[item.kind] ?? item.kind} · {formatPeriodLabel(item.period)}</p><p className="mt-1 text-xs text-muted-foreground">{due ? `Jatuh tempo ${due}` : "Tanggal perlu dikonfirmasi"} · bayar {item.paymentStatus} · data {item.dataStatus}</p>{item.deadlineStatus === "PROVISIONAL" && <p className="mt-1 text-xs font-medium text-amber-700">Tanggal provisional—kalender libur resmi belum tersedia untuk tahun ini.</p>}{item.statutoryDueDate && item.effectiveDueDate && dateOnly(item.statutoryDueDate) !== dateOnly(item.effectiveDueDate) && <p className="mt-1 text-xs text-muted-foreground">Tanggal dasar {dateLabel(item.statutoryDueDate)} digeser ke hari kerja berikutnya.</p>}</div><p className="shrink-0 font-semibold tabular-nums">{item.remainingPayable === null ? "Belum tersedia" : formatRupiah(item.remainingPayable)}</p></div>
