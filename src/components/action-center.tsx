...
export function ActionCenter() {
  const invoice = useQuery({ queryKey: ["actions", "invoice"], queryFn: getInvoiceSummary }); const reminders = useQuery({ queryKey: ["actions", "invoice-reminders"], queryFn: listInvoiceReminders }); const documents = useQuery({ queryKey: ["actions", "documents"], queryFn: () => listDocuments() }); const { data: tax } = useTaxAgenda()
