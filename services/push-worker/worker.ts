...
type Job = { id: string; endpoint: string; keys: { p256dh: string; auth: string }; payload: { title: string; body: string; url: string; tag: string } }
...
async function deliver(job: Job) { try { await webpush.sendNotification({ endpoint: job.endpoint, keys: job.keys }, JSON.stringify(job.payload), { TTL: 86_400, urgency: "normal" }); await api(`/api/jornal/internal/push/${encodeURIComponent(job.id)}/complete`, { outcome: "SENT" }) } catch (cause) { const status = Number((cause as { statusCode?: number }).statusCode || 0); const permanent = status === 404 || status === 410; await api(`/api/jornal/internal/push/${encodeURIComponent(job.id)}/complete`, { outcome: permanent ? "PERMANENT" : "RETRYABLE", error: `web-push:${status || "network"}` }).catch(() => undefined) } }
